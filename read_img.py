...
breed_list = os.listdir('./input/annotation/Annotation/')  # list of all breeds for further demo

plt.figure(figsize=(10, 10))
for i in range(9):
    plt.subplot(331 + i)  # showing 9 random images
    breed = np.random.choice(breed_list)  # random breed
    dog = np.random.choice(os.listdir('./input/annotation/Annotation/' + breed))  # random image
    img = Image.open('./input/images/Images/' + breed + '/' + dog + '.jpg')
    tree = ET.parse('./input/annotation/Annotation/' + breed + '/' + dog)  # init parser for file given
